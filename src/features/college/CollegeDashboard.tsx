...
export const CollegeDashboard: React.FC = () => {
  ...
  const [selectedRole, setSelectedRole] = useState('Placement Officer');
  ...
  // Bulk seat generator
  const [numSeats, setNumSeats] = useState(25);
  const [bulkCodes, setBulkCodes] = useState<string[]>([]);

  // Local data templates
  const [studentsList, setStudentsList] = useState([
    { id: 'ST-501', name: 'Rohan Sharma', class: 'Semester VI', dept: 'Tech Division', status: 'Completed', readiness: 85, counseling: 'Scheduled', risk: 'Low', goal: 'Software Engineer' },
    { id: 'ST-502', name: 'Ananya Sharma', class: 'Semester IV', dept: 'Business/Commerce', status: 'Completed', readiness: 72, counseling: 'Pending', risk: 'Medium', goal: 'UX Designer' },
    { id: 'ST-503', name: 'Kabir Verma', class: 'Semester II', dept: 'Management/MBA', status: 'In Progress', readiness: 45, counseling: 'Required', risk: 'High', goal: 'Investment Analyst' },
    { id: 'ST-504', name: 'Meera Sen', class: 'Semester VI', dept: 'Tech Division', status: 'Completed', readiness: 88, counseling: 'Completed', risk: 'Low', goal: 'Cloud Engineer' },
    { id: 'ST-505', name: 'Aarav Patel', class: 'Semester VI', dept: 'Management/MBA', status: 'In Progress', readiness: 30, counseling: 'Required', risk: 'High', goal: 'Product Manager' }
  ]);

  const handleGenerateSeats = () => {
    const codes = [];
    const prefix = user?.collegeName?.split(' ').map(w => w.charAt(0)).join('').toUpperCase() || 'COL';
  ...
  const placementReadinessData = [
    { name: 'Engineering/Tech', CampusScore: 78, CorporateBenchmark: 85 },
    { name: 'Business/Commerce', CampusScore: 84, CorporateBenchmark: 82 },
    { name: 'Management/MBA', CampusScore: 89, CorporateBenchmark: 90 },
  ];

  const skillGapsData = [
    { subject: 'Valuation Finance', Gap: 30 },
    { subject: 'Agile Operations', Gap: 18 },
    { subject: 'Neural Networks', Gap: 42 },
    { subject: 'Speech Delivery', Current: 25 },
  ];

  const progressOverTime = [
    { month: 'Jan', DiagnosticComplete: 50, PlacedGraduates: 10 },
    { month: 'Feb', DiagnosticComplete: 65, PlacedGraduates: 25 },
    { month: 'Mar', DiagnosticComplete: 80, PlacedGraduates: 50 },
    { month: 'Apr', DiagnosticComplete: 95, PlacedGraduates: 85 },
  ];
      ...
      <aside className={`hidden lg:flex flex-col border-r shrink-0 transition-all duration-300 ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} ${sidebarOpen ? 'w-64' : 'w-20'}`}>
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto no-scrollbar">
          {[
            ...
            { label: 'Departments', icon: GraduationCap, tab: 'classes' },
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-8">
          ...
          <div className="rounded-3xl bg-slate-900 text-white p-6 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-md text-left">
            <div className="space-y-1">
              <span className="rounded bg-brand-red px-2.5 py-1 text-[10px] font-black uppercase text-white tracking-wider">
                University Placement Administration Portal
              </span>
              <h1 className="text-2xl font-black tracking-tight">{user.collegeName || 'Torque Partner University'}</h1>
          ...
          {activeTab === 'overview' && (
            <div className="space-y-8 text-left">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { title: 'Total Enrolled Students', val: '2,800', desc: 'Active Seats', col: 'text-slate-900' },
                  { title: 'Placement readiness', val: '84%', desc: 'Assessment Index', col: 'text-slate-900' },
                  { title: 'Advisory slots', val: '58 sessions', desc: 'Scheduled This Week', col: 'text-brand-red' },
                  { title: 'Active Faculty Members', val: '72 Supervisor', desc: 'Staff Strength', col: 'text-emerald-600' },
                ].map((stat, idx) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-6">
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest">Division Placement Readiness indices (%)</h4>
                  <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={placementReadinessData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" />
                        <XAxis dataKey="name" tick={{ fill: '#475569', fontSize: 10, fontWeight: 750 }} />
                        <YAxis tick={{ fill: '#94A3B8', fontSize: 10 }} />
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="CampusScore" fill="#C62828" name="Campus Score" radius={[4, 4, 0, 0]} barSize={25} />
                        <Bar dataKey="CorporateBenchmark" fill="#1E293B" name="Corporate Benchmark" radius={[4, 4, 0, 0]} barSize={25} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>

                {/* Cohort Skill Gaps representation */}
                <div className="lg:col-span-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest border-b border-slate-100 pb-2">Campus Skill Gaps metrics</h4>
                  ...
                  <table className="w-full text-left border-collapse text-xs font-bold text-slate-700">
                    <thead>
                      <tr className="border-b border-slate-200 uppercase text-slate-400 text-[10px]">
                        ...
                        <th className="py-2.5">Division Department</th>
          ...
          {activeTab === 'classes' && (
            <div className="space-y-6 text-left">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest border-b border-slate-100 pb-2">Departments structure</h4>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    { section: 'Engineering/Tech Division', count: 1200, supervisor: 'Dr. Amitabh Roy', completion: 92 },
                    { section: 'Business/Commerce Division', count: 850, supervisor: 'Prof. Lisa Dsouza', completion: 74 },
                    { section: 'Management/MBA Division', count: 750, supervisor: 'Prof. David Miller', completion: 88 },
                  ].map((cls, idx) => (
                    <div key={idx} className="bg-slate-55 border border-slate-200 p-4.5 rounded-2xl space-y-3.5">
                      ...
                      <div className="text-[10px] font-semibold text-slate-500 space-y-1">
                        <p><strong>Faculty head:</strong> {cls.supervisor}</p>
            ...
            <div className="space-y-6 text-left">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                ...
                <div className="space-y-3.5">
                  {[
                    { name: 'MBA Pre-Placement Readiness Screening', target: 'Management Division', comp: 92 },
                    { name: 'Undergrad Cognitive Aptitude Evaluation', target: 'BBA Semester IV', comp: 65 },
                  ].map((camp, idx) => (
            ...
            <div className="space-y-6 text-left">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                ...
                <div className="space-y-3 font-semibold text-xs text-slate-650">
                  <p><strong>Assigned Advisor:</strong> Dr. Sunita Mehta, Prof. Amit Roy</p>
                  <p><strong>High priority checklist:</strong> 5 students flagged with high risk factors requiring immediate slots.</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 text-left">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                ...
                <div className="divide-y divide-slate-100 text-xs font-semibold text-slate-700">
                  {['Dr. Amitabh Roy (Tech Head)', 'Prof. Lisa Dsouza (Commerce Lead)', 'Prof. David Miller (MBA Supervisor)'].map((fac, idx) => (
                ...
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={progressOverTime}>
                      ...
                      <Line type="monotone" dataKey="PlacedGraduates" stroke="#1E293B" name="Placed Graduates" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
            ...
            <div className="space-y-6 text-left">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                ...
                <div className="divide-y divide-slate-100 text-xs font-bold text-slate-700">
                  {['Campus composite placement report', 'Department comparative index report', 'Student active completion matrix'].map((rep, idx) => (
